...
dir.output <- paste0(dir.pdf, "fetal_brain_mrna-eqtl_colocalizations/5e5/")
...
mirQTL.dir.ld <- here("results/co-localization/fetal_brain_mRNA-eQTL/ld/mirQTL/")
mQTL.dir.ld <- here("results/co-localization/fetal_brain_mRNA-eQTL/ld/mQTL/")
...
# table of overlaps
overlap.rds <- here("results/co-localization/fetal_brain_mRNA-eQTL/fetal_brain_mRNA-eQTL_mirQTL_overlaps_r2at0.8.rds")
...
df.overlaps %<>%
    mutate(overlap = paste(eQTL.mirQTL, ENSG.mQTL, sep = "+"),
...
for (i in 1:nrow(df.overlaps)) {
    ...
    df.results.ensg <- dplyr::filter(df.results.mrna, ENSG == ensg, BP >= startRange.esnp & BP <= endRange.esnp)
