...
dir.output <- paste0(dir.pdf, "gwas_coloc/")
...
# loop over unique colocs and plot
for (i in 1:nrow(df.coloc.edu.unique)) {

    esnp <- df.coloc.edu.unique$eSNP[i]
    emir <- df.coloc.edu.unique$emiR[i]
    eqtl <- df.coloc.edu.unique$eQTL[i]

    chr <- paste0("chr", df.coloc.edu.unique$CHR_A.mirQTL[i])
    pos <- df.coloc.edu.unique$BP_A.mirQTL[i]

    gwas <- "Edu_Attain"
    gwas.var <- getRSIDfrom1kgEUR(chr, pos)

    # output plot file
    output.file <- paste0(dir.output, gwas, "_", df.coloc.edu.unique$eQTL_GWAS_INDEX[i], ".pdf")
    ...
    gr.mirqtl <- GRanges(seqnames = df.mirQTL.plotting$CHR,
                       ...
                       dotColor = df.mirQTL.plotting$color)


    df.plot <- dplyr::select(df.mirQTL.plotting,
                             start = BP.hg38,
                             end = BP.hg38,
                             chromosome = CHR,
                             P)

    df.plot <- mutate(df.plot,
                      neglog10p = -log10(P))

    allTrack <- DataTrack(range = df.plot,
                          name = "miRNA -log10(P)",
                          type = "p",
                          genome = Seqinfo(genome = "hg38"))

    plotTracks(list(allTrack))
