...
# OUTPUT ##############################################################################################################
dir.pdf <- here("doc/plot_colocalization/pdfs/scz")
...
# Import GWAS Data ####################################################################################################
df.gwas <- read_tsv(here("data/gwas_datasets/scz/download/clozuk_pgc2.meta.sumstats.txt.gz"))
...
#for (i in 1:3) {
    ...
    df.gwas %>%
        filter(BP > startRange, BP < endRange) -> df.gwas.sub

    gr.gwas <- GRanges(seqnames = paste0("chr", df.gwas.sub$CHR),
                       ranges = IRanges(start = df.gwas.sub$BP,
                                        end = df.gwas.sub$BP,
                                        names = df.gwas.sub$SNP),
                       neglog10p = -log10(df.gwas.sub$P))
