# look eqtls and expressed mirnas in relation to host genes
...
# splice ratios
splice.raw.rds <- here("results/external_data/fetal_brain_sQTL/fetal_brain_sQTL_raw_ratios.rds")
splice.norm.rds <- here("results/external_data/fetal_brain_sQTL/fetal_brain_sQTL_norm_ratios.rds")
...
# Import Splice Data ###################################################################################################
df.splice.raw <- read_rds(splice.raw.rds)

df.splice.norm <- read_rds(splice.norm.rds)
...
df.expr.pri.mirna.TO.hosts %<>%
    select(ends_with(".primir"),
           ...
           expression.gene = gene.expr)

# for each expression miRNA, combine with primary/host gene information
df.mirna %<>%
    left_join(df.expr.pri.mirna.TO.hosts, by = c("Derives_from" = "ID.primir"))
...
df.mirna %>%
    filter(! is.na(gene_id)) -> df.mirna.with.host

# expression matrix for miRNA and gene expression
mat.mirna.expr <- assay(vsd.mirna)[df.mirna.with.host$uniqueName, ]
mat.gene.expr <- assay(vsd.gene)[df.mirna.with.host$gene_id, ]

# get correlation
df.mirna.with.host$corr <- sapply(1:dim(mat.mirna.expr)[1], function(i) cor(mat.mirna.expr[i,], mat.gene.expr[i,]))


# do they have eqtls
df.mirna.with.host$have.mirna.eqtl <- df.mirna.with.host$uniqueName %in% df.eqtl$emiR
df.mirna.with.host$have.mirna.eqtl.high <- df.mirna.with.host$uniqueName %in% df.eqtl$emiR[df.eqtl$SIGNIFICANCE == "eigenMT_fdr5percent"]
df.mirna.with.host$have.gene.eqtl <- df.mirna.with.host$gene_id %in% df.eqtl.gene$ENSG
df.mirna.with.host$have.gene.sqtl <- df.mirna.with.host$gene_id %in% df.sqtl.gene$ensemblID

# is there a coloc
df.mirna.with.host$have.mirna.gene.coloc <- df.mirna.with.host$uniqueName %in% df.mrna.colocs$emiR.mirQTL & df.mirna.with.host$gene_id %in% df.mrna.colocs$ENSG.mQTL
df.mirna.with.host$have.mirna.sqtl.coloc <- df.mirna.with.host$uniqueName %in% df.sqtl.colocs$emiR.mirQTL & df.mirna.with.host$gene_id %in% df.sqtl.colocs$ensemblID.mQTL

colscale <- scale_fill_manual(values = c("darkblue", "darkorange"))

df.mirna.with.host %>%
    ggplot(aes(x = corr, fill = have.mirna.eqtl)) +
...
df.sqtl.colocs %>%
    filter(! duplicated(corr.splice.raw)) %>%
    ggplot(aes(x = corr.splice.raw, fill = emir.in.sgene)) +
    geom_histogram(bins = 5) +
    scale_fill_manual(values = c("darkblue", "darkorange")) +
    plotTheme("figure")
...
ggsave("~/Desktop/eqtl_distance_hosts.pdf", height = 2, width = 2.8)
...
ggsave("~/Desktop/eqtl_colocs.pdf", height = 2, width = 3.3)
