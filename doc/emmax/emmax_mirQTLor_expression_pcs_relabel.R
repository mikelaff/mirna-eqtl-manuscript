...
# INPUT ##############
files.rds <- list.files(here("results/emmax/association_results/20191202_mirQTLor_test_covariates_expression_pcs_relabel/"), pattern = "*.rds")
...
for (df.file in files.rds) {
    ...
    df.tmp <- as_tibble(readRDS(paste0(here("results/emmax/association_results/20191202_mirQTLor_test_covariates_expression_pcs_relabel/"), df.file)))
...
df.cov.relabel <- df.cov
df.eqtls.relabel <- df.eqtls
...
df.cov.relabel$samples <- "relabeled"

df.cov <- bind_rows(select(df.cov.relabel,
                           covMat,
                           emirCount,
                           esnpCount,
                           eqtlCount,
...
ggsave(paste0(dir.pdf, "eqtls_by_relabeled_and_expressionPCA.pdf"), width = 9, height = 5)
