...
# OUTPUT FILES #########################################################################################################
dir.pdfs <- here("doc/emmax/pdfs/1kg_v_topmed_highFDR/")
...
df.1kg.hg38.filt %>%
    filter(PVAL.adj < 0.1) %>%
    top_n(1, PVAL) %>%
    pull(PVAL) -> p.value.nominal.threshold.1kg.high.fdr

df.topmed.hg38.filt %>%
    filter(PVAL.adj < 0.1) %>%
    top_n(1, PVAL) %>%
    pull(PVAL) -> p.value.nominal.threshold.topmed.high.fdr
...
for (i in 1:length(df.emirs$uniqename)) {
    ...
    if (mir %in% emiRs.1kg.only.high.fdr) {
        filename <- file.path(dir.pdfs, paste0("/1kg_only/", mir, ".pdf"))
    } else if (mir %in% emiRs.topmed.only.high.fdr) {
    ...
    if (length(gr.1kg) > 0) {
        dTrack.1kg <- DataTrack(gr.1kg,
                                name = "1KG",
                                type = "p",
                                baseline=-log10(p.value.nominal.threshold.1kg.high.fdr),
    ...
    dTrack.topmed <- DataTrack(gr.topmed,
                               name = "TOPMed",
                               type = "p",
                               baseline=-log10(p.value.nominal.threshold.topmed.high.fdr),
