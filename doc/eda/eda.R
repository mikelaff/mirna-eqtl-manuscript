...
mirbase_se_rds <- here("results/rdata_files/20190505_rse_all_known_and_novel_counts.rds")

# Build DDS #######################################################################################
# read in summarized experiment
se <- readRDS(mirbase_se_rds)

#se <- se[mcols(se)$source == "miRBase_v22",]
...
df %>%
    ggplot(aes(log10(rna_concentration), log10(reads_per_sample))) +
    geom_point() +
    plotTheme
