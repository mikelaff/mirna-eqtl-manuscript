...
fisher_test_overlap_rds <- here("doc/target_overlap/rdata/mirna_fisher_test_overlap_results.rds")
...
tmp %>%
  dplyr::filter(category == "neurogenesis_gz") %>%
...
tmp %>%
  dplyr::filter(category == "neurogenesis_cp") %>%
  ggplot(aes(x=log10(FT_gz_cp_odds), y=-log10(FT_gz_cp_pval))) +
  geom_point() +
  plotTheme +
  geom_hline(yintercept = -log10(0.05))
...
tmp %>%
  dplyr::filter(category == "maturation_early") %>%
...
tmp %>%
  dplyr::filter(category == "maturation_late") %>%
  ggplot(aes(x=FT_early_late_odds, y=-log10(FT_early_late_pval))) +
  geom_point() +
  plotTheme +
  geom_hline(yintercept = -log10(0.05))
