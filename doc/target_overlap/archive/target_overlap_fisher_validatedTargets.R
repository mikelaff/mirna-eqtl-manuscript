...
fisher_test_overlap_rds <- here("doc/target_overlap/rdata/mirna_fisher_test_overlap_results_valTargets.rds")
...
for (i in indexes) {
  ...
  # get valid targets
  predictions <- filter(targets, mirna == mirna_df$mir[i])$ENSEMBL
  ...
  # number of targets overlapping none category (not diff. expressed, but present in dataset)
  mirna_df$num_targets_notDE[i] <- sum(predictions %in% genes_notDE)
