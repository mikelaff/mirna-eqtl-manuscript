...
df.logistic.reg.mirdb.rds <- paste(paste(here("doc/target_overlap/rdata/"),
                                                    date.prefix, "_miRDB_logistic_regression_log10tx.rds", sep=""))
...
for (p in 1:length(pred.list)) {
  ...
  for (cluster in clusters) {
    ...
    for (i in 1:length(df.mirna.enrichments$Name)) {
      ...
      lg <- glm(cluster_specific ~ target + log10tx, data = df.genes, family = binomial)
...
dfM.lg <- readRDS(here("doc/target_overlap/rdata/20190619_miRDB_logistic_regression.rds"))
dfM.ft <- readRDS(here("doc/target_overlap/rdata/20190530_miRDB_known_novel_enrichments.rds"))

pdf(paste("~/Desktop/", "heatmap_miRDB2019_lg_matur.pdf"), height=10, width=10, useDingbats = FALSE)
#plot.enrichments(dfM.ft, categories = c("NeuroGZ", "NeuroCP"), title = "Fisher, miRDB2019")
plot.lg.enrichments(dfM.lg, categories = c("MatEarly", "MatLate"), title = "Log. Regression (+ mean tx length), miRDB2019")
dev.off()

df2.lg <- readRDS(here("doc/target_overlap/rdata/20190619_miRNAtap_2sources_logistic_regression.rds"))
df2.ft <- readRDS(here("doc/target_overlap/rdata/20190530_miRNAtap_enrichments_2sources.rds"))

pdf(paste(dir.pdf, "heatmap_2sources_ft_lg_comparison.pdf"), height=10, width=10, useDingbats = FALSE)
plot.enrichments(df2.ft, categories = c("NeuroGZ", "NeuroCP"), title = "Fisher, 2sources")
plot.lg.enrichments(df2.lg, categories = c("NeuroGZ", "NeuroCP"), title = "Log. Regression (+ mean tx length), 2sources")
dev.off()

dfM.lg <- readRDS(here("doc/target_overlap/rdata/20190620_miRDB_logistic_regression_3utr.rds"))
dfM.ft <- readRDS(here("doc/target_overlap/rdata/20190530_miRDB_known_novel_enrichments.rds"))

pdf(paste(dir.pdf, "heatmap_miRDB2019_ft_lg_comparison.pdf"), height=10, width=10, useDingbats = FALSE)
plot.enrichments(dfM.ft, categories = c("NeuroGZ", "NeuroCP"), title = "Fisher, miRDB2019")
plot.lg.enrichments(dfM.lg, categories = c("NeuroGZ", "NeuroCP"), title = "Log. Regression (+ mean 3utr length), miRDB2019")
dev.off()

df2.lg <- readRDS(here("doc/target_overlap/rdata/20190627_miRNAtap_2sources_logistic_regression_log10tx.rds"))
df3.lg <- readRDS(here("doc/target_overlap/rdata/20190627_miRNAtap_3sources_logistic_regression_log10tx.rds"))
df4.lg <- readRDS(here("doc/target_overlap/rdata/20190627_miRNAtap_4sources_logistic_regression_log10tx.rds"))

pdf(paste(dir.pdf, "heatmap_2_3_4scources_lg_neuro_log10tx.pdf"), height=10, width=10, useDingbats = FALSE)
plot.lg.enrichments(df2.lg, categories = c("NeuroGZ", "NeuroCP"), title = "Log. Regression (+ log10tx), 2sources")
plot.lg.enrichments(df3.lg, categories = c("NeuroGZ", "NeuroCP"), title = "Log. Regression (+ log10tx), 3sources")
plot.lg.enrichments(df4.lg, categories = c("NeuroGZ", "NeuroCP"), title = "Log. Regression (+ log10tx), 4sources")
dev.off()

pdf(paste(dir.pdf, "heatmap_2_3_4scources_lg_matur_log10tx.pdf"), height=10, width=10, useDingbats = FALSE)
plot.lg.enrichments(df2.lg, categories = c("MatEarly", "MatLate"), title = "Log. Regression (+ log10tx), 2sources")
plot.lg.enrichments(df3.lg, categories = c("MatEarly", "MatLate"), title = "Log. Regression (+ log10tx), 3sources")
plot.lg.enrichments(df4.lg, categories = c("MatEarly", "MatLate"), title = "Log. Regression (+ log10tx), 4sources")
dev.off()
