...
df.logistic.reg.mirdb.rds <- paste(paste(here("doc/target_overlap/rdata/"),
                                                    date.prefix, "_miRDB_logistic_regression_log10tx_reverse.rds", sep=""))
...
for (p in 1:length(pred.list)) {
  ...
  for (cluster in clusters) {
    ...
    for (i in 1:length(df.mirna.enrichments$Name)) {
      ...
      lg <- glm(target ~ cluster_specific + log10tx, data = df.genes, family = binomial)

      coef.lg <- coef(lg)

      # Fill in df.lg: coefficient for "cluster_specific1"
      df.lg$LGlogit[i] <- coef.lg["cluster_specific1"]
      df.lg$LGoddsRatio[i] <- exp(coef.lg)["cluster_specific1"]
      df.lg$LGprob[i] <- (exp(coef.lg) / (1 + exp(coef.lg)))["cluster_specific1"]
      df.lg$LGpval[i] <- coef(summary(lg))["cluster_specific1", 4]
    }
...
dfM.lg <- readRDS(here("doc/target_overlap/rdata/20190624_miRDB_logistic_regression_log10tx.rds"))

pdf(paste(dir.pdf, "heatmap_miRDB2019_ft_lg_comparison_maturation_log10tx.pdf"), height=10, width=10, useDingbats = FALSE)
plot.enrichments(dfM.ft, categories = c("MatEarly", "MatLate"), title = "Fisher, miRDB2019")
plot.lg.enrichments(dfM.lg, categories = c("MatEarly", "MatLate"), title = "Log. Regression (cluster_specific ~ target + log10tx), miRDB2019")
dev.off()
...
#
# pdf(paste(dir.pdf, "heatmap_miRDB2019_ft_lg_comparison_3utrlength.pdf"), height=10, width=10, useDingbats = FALSE)
