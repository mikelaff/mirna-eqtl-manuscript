...
dir.output <- here("doc/mirQTL_plots/pdfs/mirQTL_ld_snps_preregress/")
...
# eqtls in the "final pipeline" with emmax doing the regressing
df.eqtls.fp <- as_tibble(readRDS(eqtls.dataframe.fp.rds))
df.eqtls.fp %<>%
    mutate(eqtl = paste(emir, esnp, sep = "+"))

preregress.only <- df.eqtls$eqtl[ ! df.eqtls$eqtl %in% df.eqtls.fp$eqtl ]
both <- df.eqtls$eqtl[ df.eqtls$eqtl %in% df.eqtls.fp$eqtl ]

print("Importing association results...")
...
print("Finished importing association results.")
...
print("Finished importing expression data.")
