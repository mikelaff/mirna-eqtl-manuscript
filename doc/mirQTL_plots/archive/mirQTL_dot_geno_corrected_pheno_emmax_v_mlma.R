...
dir.output <- here("doc/mirQTL_plots/pdfs/mirQTL_dot_geno_corrected_pheno_emmax_v_mlma/")
...
for (i in 1:nrow(df.eqtls)) {
    ...
    pushViewport(viewport(layout = grid.layout(nrow = 6, ncol = 5, heights = unit(c(0.5, 5, 5, 5, 5, 5), "null"))))

    grid.text(plot.title, vp = viewport(layout.pos.row = 1, layout.pos.col = 1:5))

    pushViewport(viewport(layout.pos.row = 2, layout.pos.col = 1:5))

    tryCatch(
        {
            plotTracks(list(itrack, gtrack, grtrack.emir, miRTrack),
                       chromosome = chr,
    ...
    plotTracks(list(dTrack.emmax.fp.emir),
               chromosome = chr,
               from = startRange.emir,
               to = endRange.emir,
               transcriptAnnotation="symbol",
