...
dir.output <- here("doc/mirQTL_plots/pdfs/mirQTL_dot_geno_pheno/")
...
for (i in inds) {
    ...
    pushViewport(viewport(layout = grid.layout(nrow = 3, ncol = 4, heights = unit(c(0.5, 5, 5), "null"))))

    grid.text(plot.title, vp = viewport(layout.pos.row = 1, layout.pos.col = 1:4))
