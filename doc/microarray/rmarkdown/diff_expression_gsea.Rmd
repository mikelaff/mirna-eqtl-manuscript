---
title: "HNP Differentiation Microarray Differential Expression: GSEA"
...
```{r}
df.results %>%
    pull(t.expression_in_week2) -> gene.list

df.results %>%
    pull(SYMBOL) -> names(gene.list)
...
dotplot(gse, showCategory=10, split=".sign") + facet_grid(.~.sign) + theme(axis.text.y.left = element_text(size = 4))
dotplot(gse)
...
df.results %>%
    select(SYMBOL, t.diff_in_control) -> df.export

sum(duplicated(df.export$SYMBOL))

write_tsv(df.export, file = "~/Desktop/diff_in_control.tsv")


df.results %>%
    select(SYMBOL, t.expression_in_week2) -> df.export

sum(duplicated(df.export$SYMBOL))

write_tsv(df.export, file = "~/Desktop/expression_in_week2.tsv")


df.results %>%
    pull(t.diff_in_control) -> gene.list
