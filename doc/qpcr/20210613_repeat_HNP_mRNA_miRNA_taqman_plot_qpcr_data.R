...
df.data %>%
    ...
    labs(y = "mean(CT)",
         x = "Day",
         caption = "Donor 88. HNP expression after 4-8 days.",
         title = "qPCR after HNP Lentivirus Transductions: REPEAT")
...
ggsave(paste0(dir.pdfs, "D88_ct_mean_A260_A230_repeat.pdf"), height = 8, width = 8)

df.data %>%
    ggplot(aes(y = CT_mean, x = Day)) +
    geom_boxplot(mapping = aes(shape = Expression), position = position_dodge(width = 0.85), lwd = 0.3, outlier.shape = NA) +
    geom_point(mapping = aes(shape = Expression, color = Conc_ng_ul), position = position_dodge(width = 0.85), size = 2.5) +
    facet_wrap(~Target, scales = "free_y") +
    theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) +
    scale_color_gradient(low = "blue", high = "red") +
    labs(y = "mean(CT)",
...
ggsave(paste0(dir.pdfs, "D88_ct_mean_Conc_repeat.pdf"), height = 8, width = 8)

df.taqman %>%
    ggplot(aes(y = CT_mean, x = Expression, shape = Expression)) +
    geom_boxplot(position = position_dodge(width = 0.85), lwd = 0.3) +
    geom_point(mapping = aes(color = A260_A230), position = position_dodge(width = 0.85), size = 2.5) +
    facet_wrap(~Target) +
    theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) +
    #my.theme +
    scale_color_gradient(low = "blue", high = "red") +
...
ggsave(paste0(dir.pdfs, "D88_miRNA_ct_mean_Conc.pdf"), height = 4, width = 7)

df.data %>%
    filter(Target == "ACTB") %>%
    ggplot(aes(x = CT_mean, y = A260_A230, color = Conc_ng_ul)) +
    geom_point()
