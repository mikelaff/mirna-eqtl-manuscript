...
# loop over donors, calculate delta delta CT to Lenti-miR-4707-3p-REF(C) of that donor for each target
...
for (donor in donors) {
    ...
    for (target in targets) {
        ...
        delta.ct.control <- df.tmp2$delta_CT_ACTB[match("Lenti-miR-4707-3p-REF(C)", df.tmp2$Expression)]
...
ggsave("20210111_mRNA_qPCR_ACTB_try_to_4707.pdf", height = 8, width = 8)
