...
ggsave(paste0(dir.pdfs, "20210630_miRNA_fold_change.pdf"), height = 4, width = 7)
...
ggsave(paste0(dir.pdfs, "20210630_prolif_miRNA_fold_change.pdf"), height = 2, width = 3)
...
df.taqman %>%
    ...
    scale_color_manual(values = c(paperBlue, paperRed)) +
    plotTheme("figure") +
    stat_compare_means(label.x=2, label.y=.25, size = 3, label = "p.format")

ggsave(paste0(dir.pdfs, "20210630_prolif_miRNA_fold_change_sig.pdf"), height = 2, width = 3)
...
ggsave(paste0(dir.pdfs, "20210630_prolif_miRNA_ctMean.pdf"), height = 2, width = 2.9)
...
ggsave(paste0(dir.pdfs, "20210630_miRNA_ct_mean.pdf"), height = 4, width = 7)
...
ggsave(paste0(dir.pdfs, "20210630_mRNA_fold_change_eif4a2.pdf"), height = 8, width = 8)
...
ggsave(paste0(dir.pdfs, "20210630_mRNA_fold_change_eif4a2_select.pdf"), height = 5, width = 9)
...
ggsave(paste0(dir.pdfs, "20210630_ct_mean.pdf"), height = 8, width = 8)
...
# nanodrop values
df.nano <- read_xlsx(here("results/qpcr/20210629_RNA_extractions.xlsx"), sheet = 1)
...
df.data %>%
    ggplot(aes(y = CT_mean, x = Expression)) +
    geom_boxplot(mapping = aes(shape = Donor), position = position_dodge(width = 0.85), lwd = 0.3, outlier.shape = NA) +
    geom_point(mapping = aes(shape = Donor, color = A260_A230), position = position_dodge(width = 0.85), size = 2.5) +
    facet_wrap(~Target, scales = "free_y") +
    theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) +
...
ggsave(paste0(dir.pdfs, "20210630_ct_mean_A260_A230.pdf"), height = 8, width = 8)

df.data %>%
    ggplot(aes(y = CT_mean, x = Expression)) +
    geom_boxplot(mapping = aes(shape = Donor), position = position_dodge(width = 0.85), lwd = 0.3, outlier.shape = NA) +
    geom_point(mapping = aes(shape = Donor, color = Conc_ng_ul), position = position_dodge(width = 0.85), size = 2.5) +
    facet_wrap(~Target, scales = "free_y") +
    theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) +
    scale_color_gradient(low = "blue", high = "red") +
    labs(y = "mean(CT)",
...
ggsave(paste0(dir.pdfs, "20210630_ct_mean_Conc.pdf"), height = 8, width = 8)

df.taqman %>%
    ggplot(aes(y = CT_mean, x = Expression, shape = Donor)) +
    geom_boxplot(position = position_dodge(width = 0.85), lwd = 0.3) +
    geom_point(mapping = aes(color = Conc_ng_ul), position = position_dodge(width = 0.85), size = 2.5) +
    facet_wrap(~Target) +
    theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) +
    #my.theme +
    scale_color_gradient(low = "blue", high = "red") +
...
ggsave(paste0(dir.pdfs, "20210630_miRNA_ct_mean_Conc.pdf"), height = 4, width = 7)
