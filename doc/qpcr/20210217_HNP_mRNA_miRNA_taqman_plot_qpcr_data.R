...
# miRNA qPCR TaqMan data from 17 Feb 2021
miRNA.taqman.data.xlsx <- here("results/qpcr/20210217_HNP_miRNA_TaqMan.xlsx")
...
df.data %>%
    filter(Target == "HAUS4-1" | Target == "HAUS2-1" | Target == "HAUS7-1") %>%
    filter(Expression == "Control" | Expression == "HAUS4") %>%
    ggplot(aes(y = fold_change_EIF4A2, x = Expression, color = Donor)) +
    geom_boxplot(position = position_dodge(width = 0.85), lwd = 0.3) +
    geom_point(position = position_dodge(width = 0.85)) +
    facet_wrap(~Target, scales = "free_y") +
...
ggsave("~/Desktop/haus4_OE_haus_genes.pdf", width = 12, height = 6)

df.data %>%
    filter(Target == "HAUS4-1" | Target == "Ki67-1") %>%
    filter(Expression == "Control" | Expression == "HAUS4") %>%
    ggplot(aes(y = fold_change_EIF4A2, x = Expression, color = Donor)) +
    geom_boxplot(position = position_dodge(width = 0.85), lwd = 0.3) +
    geom_point(position = position_dodge(width = 0.85)) +
...
ggsave("~/Desktop/haus4_OE_ki67.pdf", width = 8, height = 6)
...
df.mirna %>%
    ...
    labs(y = "Fold Change",
         ...
         title = "QIAGEN: qPCR after HNP Lentivirus Transductions")
