# format Alzheimer's Disease (AD) GWAS data
...
# OUTPUT FILES #########################################################################################################
output.granges.rds <- here("data/gwas_datasets/ad/ad.hg38.GRanges.rds")

# INPUT FILES ##########################################################################################################

raw1.data <- here("data/gwas_datasets/ad/download/Kunkle_etal_Stage1_results.txt?file=1")
raw2.data <- here("data/gwas_datasets/ad/download/Kunkle_etal_Stage2_results.txt?file=1")
...
df.raw1 <- read_delim(raw1.data, delim = " ")

df.raw2 <- read_delim(raw2.data, delim = " ")

df.raw <- bind_rows(df.raw1, df.raw2)
rm(df.raw1, df.raw2)
