...
coloc.output.rds <- here("results/external_data/educational_attainment/educational_attainment_mirQTL_colocalizations.rds")

# working directory for temp plink files
dir.working <- here("results/external_data/educational_attainment/")

dir.create(dir.working, recursive = TRUE, showWarnings = FALSE)

# INPUT FILES ##########################################################################################################
# educational attainment GWAS data
gr.gwas.rds <- here("data/gwas_datasets/educational_attainment/educational_attainment.hg38.GRanges.rds")
...
for (i in 1:nrow(df.mirQTL.eqtls)) {
    ...
    # write clumpFile
    write_tsv(tibble(SNP = gr.gwas.window$RSID, P = gr.gwas.window$Pval), clumpFile)
    # plink command
    ...
    print(paste0("Total colocalizations found so far: ", nrow(df.coloc)))
}
