...
# OUTPUT FILES #########################################################################################################
gwas.dataset <- "enigma3"

# working directory for temp plink files
dir.working <- paste0(here("results/external_data/"), gwas.dataset, "/surface_area.global/")
dir.create(dir.working, recursive = TRUE, showWarnings = FALSE)

# table of colocalizations
coloc.output.rds <- paste0(dir.working, gwas.dataset, ".surface_area.global_mirQTL_colocalizations.rds")

# INPUT FILES ##########################################################################################################
# GWAS data
gr.gwas.rds <- paste0(here("data/gwas_datasets/"), gwas.dataset, "/ENIGMA3_Global/", gwas.dataset, ".surface_area.global.hg38.GRanges.rds")
