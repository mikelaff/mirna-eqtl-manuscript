# find co-localization miRNA-eQTL to fetal brain mRNA-eQTLs

# for each miRNA-eQTL:
#       look for overlap with mRNA-eQTL at r2 >= 0.8
...
dir.working <- paste0(here("results/co-localization/"), "fetal_brain_mRNA-eQTL", "/")
dir.create(dir.working, recursive = TRUE, showWarnings = FALSE)

# table of overlaps
overlap.output.rds <- paste0(dir.working, "fetal_brain_mRNA-eQTL", "_mirQTL_overlaps_r2at0.8.rds")

# list of miR-eQTL index snps that have overlaps
index.snps.txt <- paste0(dir.working, "fetal_brain_mRNA-eQTL", "_overlap.index.snps.txt")
...
# mQTL eQTLs
mQTL.df.rds <- here("results/external_data/fetal_brain_mRNA-eQTL/fetal_brain_mRNA-eQTLs.rds")
...
mQTL.ld.dir <- here("results/external_data/fetal_brain_mRNA-eQTL/ld/")
...
df.mQTL %<>%
    mutate(eQTL = paste(SNP, ENSG, sep = "+"))
...
for (i in 1:nrow(df.mirQTL)) {
    ...
    for (j in 1:nrow(df.mQTL.possible)) {
        ...
        mQTL.ld.file <- paste0(mQTL.ld.dir, "mQTL.index.", df.mQTL.possible$SNP[j], ".ld")
