...
dir.output <- here("data/garfield-data/pval/huan2015_plusNonSig/")
...
for (chrom in CHROMS) {
    ...
    # load 1kg eur variants
    df.vars <- read_table2(paste0(maftssd.dir, chrom),
                           col_names = c("pos", "maf", "dist"),
                           col_types = "idi")

    df.pvals <- dplyr::filter(df.hg38, chr == chrom)

    df.output <- left_join(df.vars, df.pvals, by = "pos")
    df.output$pval[is.na(df.output$pval)] <- 1.0
