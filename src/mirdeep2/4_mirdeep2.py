...
#globals longleaf
mirdeep_output_root = "/proj/steinlab/projects/miRNA-eQTL-project/mirna-eqtl/mirdeep2/output/"
fastq_file_list = "/proj/steinlab/projects/miRNA-eQTL-project/mirna-eqtl/lists/fastq_files_fullpath.txt"
...
#for i in range(1, 5):
    ...
    with open("slurm_{}.sh".format(rnaid[i]), "w") as f:
        ...
        #mapper
        f.write("jid3=$(ssub --notify=OFF -d afterok:$jid2 --mem=8g --wrap=\\\"mapper.pl {}.fastq -e -h -j -k TGGAATTCTCGGGTGCCAAGG -l 18 -m -p /proj/steinlab/projects/miRNA-eQTL-project/mirna-eqtl/genome/hg38_UCSC/bowtie_index/hg38 -s {}_collapsed.fa -t {}_collapsed_vs_genome.arf -v\\\")\n".format(rnaid[i], rnaid[i], rnaid[i]))

        #quantifier
        f.write("jid4=$(ssub --notify=OFF -d afterok:$jid3 --mem=8g --wrap=\\\"quantifier.pl -p /proj/steinlab/projects/miRNA-eQTL-project/mirna-eqtl/mirbase/hsa_hairpin.fa -m /proj/steinlab/projects/miRNA-eQTL-project/mirna-eqtl/mirbase/hsa_mature.fa -r {}_collapsed.fa -t hsa\\\")\n".format(rnaid[i]))
