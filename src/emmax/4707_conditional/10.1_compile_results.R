...
output.dir <- paste0(here("results/emmax/association_results/"), results.name, "/compiled/")
dir.create(output.dir, recursive = TRUE, showWarnings = FALSE)

# output GRangesList RDS file
output.grl.rds <- paste0(output.dir, results.name, "_variants_GRangesList.rds")

# output data frame RDS file
output.df.rds <- paste0(output.dir, results.name, "_variants_dataFrame.rds")

# nominal p-value output text file
output.nom.p.value.file <- paste0(output.dir, results.name, "_nomPvalue.txt")

# INPUT FILES #########################################################################################################
# emmax result files directory, column labeled
results.dir <- paste0(here("results/emmax/association_results/"), results.name, "/ps/")

# mirs in this analysis
genes.tsv <- here("results/emmax/phenotype_files/20200120_mirQTLor_VST_miRNA_expression_residual/20200120_mirQTLor_genes_hg38.txt")
...
for (i in 1:nrow(genes)) {
    ...
    ps.file <- paste0(results.dir, chr, ".dosage.prefiltered.mirQTLor.", uniName, ".ps")
