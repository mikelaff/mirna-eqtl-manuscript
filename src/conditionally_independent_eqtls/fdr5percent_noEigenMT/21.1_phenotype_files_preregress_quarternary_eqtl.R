...
# OUTPUT FILES #########################################################################################################
# output directory
output.dir <- paste0(here("results/conditional_eqtls/"), results.name, "/quarternary/", results.name, "_VST_miRNA_expression_residual_after_quarternary_eqtl/")
...
vsd.output.rds <- paste0(output.dir, results.name, "_VST_miRNA_expression_and_residual_after_quarternary_eqtl_rse.rds")
...
# data frame of quarternary eqtls
...
dir.primary.genotypes <- here("results/conditional_eqtls/20200120_mirQTLor_fdr5percent_noEigenMT/primary/primary_eSNP_genotypes/")

# directory with secondary eqtl genotypes
dir.secondary.genotypes <- here("results/conditional_eqtls/20200120_mirQTLor_fdr5percent_noEigenMT/secondary/secondary_eSNP_genotypes/")

# directory with tertiary eqtl genotypes
dir.tertiary.genotypes <- here("results/conditional_eqtls/20200120_mirQTLor_fdr5percent_noEigenMT/tertiary/tertiary_eSNP_genotypes/")

# directory with quarternary eqtl genotypes
dir.quarternary.genotypes <- here("results/conditional_eqtls/20200120_mirQTLor_fdr5percent_noEigenMT/quarternary/quarternary_eSNP_genotypes/")
...
# quarternary eqtls
df.quarternary.eqtls <- read_rds(quarternary.eqtls.rds)

# filter df.joined for secondary eqtl emiRs
df.joined %<>%
    filter(UniqueName %in% df.quarternary.eqtls$UniName)
...
# covariates
batchVars.auto <- c(colnames(cov.mat.auto)[4:length(colnames(cov.mat.auto))], "primaryDosage", "secondaryDosage", "tertiaryDosage", "quarternaryDosage")
...
for (chr in CHROMS) {
    ...
    for (mir in mirs) {
        ...
        # import quarternary sample genotypes
        esnp.quarternary <- df.quarternary.eqtls$SNP[which(df.quarternary.eqtls$UniName == mir)]
        df.quarternary.genotypes <- read_table2(paste0(dir.quarternary.genotypes, esnp.quarternary, ".raw"))
        df.quarternary.genotypes %<>%
            select(DNAID = IID, quarternaryDosage = 7)
        ...
        df %<>%
            ...
            left_join(df.quarternary.genotypes, by = "DNAID")
