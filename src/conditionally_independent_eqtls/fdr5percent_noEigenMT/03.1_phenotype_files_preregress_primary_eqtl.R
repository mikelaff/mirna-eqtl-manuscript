...
# OUTPUT FILES #########################################################################################################
# output directory
output.dir <- paste0(here("results/conditional_eqtls/"), results.name, "/primary/", results.name, "_VST_miRNA_expression_residual_after_primary_eqtl/")
...
vsd.output.rds <- paste0(output.dir, results.name, "_VST_miRNA_expression_and_residual_after_primary_eqtl_rse.rds")
...
# filter df.joined for primary eqtl emiRs
df.joined %<>%
    filter(UniqueName %in% df.primary.eqtls$UniName)
...
# covariates
batchVars.auto <- c(colnames(cov.mat.auto)[4:length(colnames(cov.mat.auto))], "primaryDosage")
...
for (chr in CHROMS) {
    ...
    for (mir in mirs) {
        ...
        # filename
        filename <- file.path(path, paste0(chr, ".mirQTLor.", mir, ".secondary.pheno"))
        ...
        esnp <- df.primary.eqtls$SNP[which(df.primary.eqtls$UniName == mir)]
        df.genotypes <- read_table2(paste0(dir.primary.genotypes, esnp, ".raw"))
        df.genotypes %<>%
        ...
        df %<>%
            ...
            left_join(df.genotypes, by = "DNAID")
