...
# OUTPUT FILES #########################################################################################################
# output directory
output.dir <- paste0(here("results/conditional_eqtls/"), results.name, "/secondary/", results.name, "_VST_miRNA_expression_residual_after_secondary_eqtl/")
...
vsd.output.rds <- paste0(output.dir, results.name, "_VST_miRNA_expression_and_residual_after_secondary_eqtl_rse.rds")
...
df.joined %<>%
    filter(UniqueName %in% df.secondary.eqtls$UniName)
...
# covariates
batchVars.auto <- c(colnames(cov.mat.auto)[4:length(colnames(cov.mat.auto))], "primaryDosage", "secondaryDosage")
